...
t %>% ggplot(aes(FL_DATE, avgDepDelay)) + 
  geom_line() + 
  labs(title = 'Departure Delays in 2017', x = '', y = 'Avg. Departure Delay')

t %>% ggplot(aes(FL_DATE, avgArrivDelay)) + 
  geom_line() + 
...
```{r}
spirit.delays <- flights %>% filter(CARRIER=='NK') %>% group_by(FL_DATE) %>% summarise(avgDepDelay=mean(DEP_DELAY))

virgin.delays <- flights %>% filter(CARRIER=='VX') %>% group_by(FL_DATE) %>% summarise(avgDepDelay=mean(DEP_DELAY))

par(mfrow=c(1,2))

spirit.delays %>% ggplot(aes(FL_DATE, avgDepDelay)) + 
  geom_line() + 
  labs(title = 'Spirit Departure Delays', x = '', y = 'Avg. Departure Delay')

virgin.delays %>% ggplot(aes(FL_DATE, avgDepDelay)) + 
  geom_line() + 
